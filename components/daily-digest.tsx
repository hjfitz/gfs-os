...
import { AlertTriangle, GitPullRequest, Terminal, Zap, ArrowRight } from "lucide-react"
...
export function DailyDigest({ repos, workflowRuns }: DailyDigestProps) {
  ...
  // Calculate total PRs
  const totalPRs = repos.reduce((acc, repo) => acc + repo.pullRequests.openCount, 0)

  // Find failing workflows
  const failingWorkflows = workflowRuns.filter((wr) => wr.run.status === "completed" && wr.run.conclusion === "failure")

  // Get the most recent failing workflow
  const mostRecentFailure =
    failingWorkflows.length > 0
      ? failingWorkflows.sort((a, b) => new Date(b.run.updatedAt).getTime() - new Date(a.run.updatedAt).getTime())[0]
      : null
  ...
  const getResolutionSuggestions = (failure: WorkflowRunWithDetails) => {
    ...
    const failingJob = failure.jobs.find((job) => job.status === "completed" && job.conclusion === "failure")
    ...
    <Card className="bg-zinc-900/40 border-zinc-800 mb-8 overflow-hidden">
      ...
      <CardHeader className="pb-2">
        <div className="flex justify-between items-start">
          <CardTitle className="text-zinc-100 font-mono text-xl">Daily Digest</CardTitle>
          <Badge variant="outline" className="font-mono text-xs bg-zinc-800/50 text-zinc-400">
            {formattedDate}
          </Badge>
        </div>
      </CardHeader>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-zinc-800/30 rounded-md p-4">
              <div className="flex items-start">
                ...
                <div>
                  <h4 className="text-zinc-300 font-medium mb-1">Pull Requests</h4>
                  <p className="text-zinc-500 text-sm mb-2">
                    You have <span className="text-zinc-300 font-mono">{totalPRs}</span> open pull requests across{" "}
            ...
            <div className="bg-zinc-800/30 rounded-md p-4">
              <div className="flex items-start">
                ...
                />
                <div>
                  <h4 className="text-zinc-300 font-medium mb-1">Workflow Status</h4>
                  <p className="text-zinc-500 text-sm mb-2">
                    {failingWorkflows.length > 0 ? (
                      <span>
                        There are <span className="text-red-400 font-mono">{failingWorkflows.length}</span> failing
                        workflows that need attention.
                      </span>
                    ) : (
                      <span>
                        All workflows are currently <span className="text-emerald-400">passing</span>. Great job!
                      </span>
                    )}
                  </p>
                  <Link
          ...
          {mostRecentFailure && (
            <div className="bg-red-950/20 border border-red-900/30 rounded-md p-4">
              <div className="flex items-start">
                ...
                <div>
                  <h4 className="text-zinc-200 font-medium mb-1">Recent Failure Analysis</h4>
                  <p className="text-zinc-400 text-sm mb-3">
                    <span className="text-red-400 font-mono">{mostRecentFailure.repo.name}</span> workflow
                    <span className="text-zinc-300 font-mono"> {mostRecentFailure.run.name}</span> failed{" "}
                    {formatDistanceToNow(new Date(mostRecentFailure.run.updatedAt))} ago.
                  </p>
                  ...
                  <div className="space-y-2 mb-3">
                    ...
                    <ul className="space-y-1 pl-4">
                      {getResolutionSuggestions(mostRecentFailure).map((suggestion, index) => (
                        <li key={index} className="text-zinc-400 text-xs list-disc">
                          {suggestion}
                        </li>
                      ))}
                    </ul>
