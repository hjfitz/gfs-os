...
export const RepositoryCard = ({ repo }: RepositoryCardProps) => (
	<Card className="bg-zinc-900/30 border border-zinc-800 rounded-lg hover:bg-zinc-800/50 transition-colors overflow-hidden group">
		<div
			className={`h-1 w-full ${repo.workflow.status ? "bg-emerald-500" : "bg-red-500"}`}
		/>
		<CardHeader className="pb-2">
			<div className="flex justify-between items-start">
				<CardTitle className="text-zinc-100 font-mono text-lg truncate">
					{repo.name}
				</CardTitle>
				<Badge
					variant="outline"
					className="font-mono text-xs bg-zinc-800 text-zinc-300 hover:text-zinc-300"
				>
				...
				<div className="bg-zinc-800/50 rounded-md p-3 flex flex-col">
					...
					<div className="flex items-center">
						...
						<span className="text-zinc-200 font-mono">
							{repo.workflow.status ? "Passing" : "Failing"}
						</span>
					</div>
				</div>
			</div>
		...
		<CardFooter className="pt-0 text-zinc-400 text-xs font-mono flex justify-between">
			<div className="flex items-center">
				...
				<span>
					{repo.lastCommit
						? `Updated ${formatDistanceToNow(new Date(repo.lastCommit))} ago`
						: "No recent commits"}
				</span>
			</div>
