	...
	return (
		<div className="space-y-6">
			{pullRequestsByRepo
				...
				.map(({ repo, openPullRequests, pullRequests }) => (
					<div
						key={repo}
						...
						<div className="flex items-center justify-between p-4 border-b border-zinc-800">
							<div className="flex items-center">
								<div className="font-mono text-zinc-100 text-lg flex items-center">
									{repo}
									<Badge className="ml-3 bg-blue-500/20 text-blue-400 border-none">
										{pullRequests.length}
									</Badge>
								</div>
							</div>
							<div className="text-zinc-400 text-sm">
								<span className="font-mono">{openPullRequests}</span> pending
								pull {openPullRequests === 1 ? "request" : "requests"}
							</div>
						</div>
