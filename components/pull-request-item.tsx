import { User, Calendar, ArrowUpDown, GitPullRequest } from "lucide-react"
  ...
  >
    <div className="p-4 border-b border-zinc-800 last:border-b-0 hover:bg-zinc-800/30 transition-colors">
      <div className="flex items-start gap-3">
        <GitPullRequest className={`h-5 w-5 mt-1 ${pr.isDraft ? "text-zinc-500" : "text-blue-400"}`} />
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-3 text-xs text-zinc-400 mt-2">
            <div className="flex items-center">
              ...
              <div className="flex items-center">
                <Avatar className="h-4 w-4 mr-1">
                  <AvatarImage src={pr.author.avatarUrl || "/placeholder.svg"} alt={pr.author.login} />
            ...
            <div className="flex items-center">
              ...
              <span>Opened {formatDistanceToNow(new Date(pr.createdAt))} ago</span>
            </div>
          </div>
        </div>
        <div className="flex flex-col items-end gap-2">
          <Badge
            variant="outline"
            className={`font-mono text-xs ${pr.isDraft
