...
export const WorkflowFilterToggle = ({
	workflowRuns,
	onFilterChange,
	initialValue,
}: WorkflowFilterToggleProps) => {
	// Check if there are any failed workflows
	const hasFailures = workflowRuns.some(
	...
	const [showFailuresOnly, setShowFailuresOnly] = useState(
		initialValue ?? hasFailures,
	);
	...
	return (
		<div className="flex items-center justify-between bg-zinc-900/40 border border-zinc-800 rounded-lg p-4 mb-6">
			<div className="flex items-center">
				<AlertTriangle
					className={`h-5 w-5 mr-3 ${hasFailures ? "text-red-400" : "text-zinc-500"}`}
				/>
				<div>
					<h3 className="text-zinc-200 font-medium">Workflow Failures</h3>
					<p className="text-zinc-500 text-sm">
						{hasFailures
							? "There are failed workflows in your repositories"
