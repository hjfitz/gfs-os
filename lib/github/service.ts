...
export class GithubService {
	...
	public async getPullRequests(): Promise<
		{ repo: string; openPullRequests: number; pullRequests: PullRequest[] }[]
	> {
	...
	public async getRepo(repo: GithubOrgRepo): Promise<RepoData> {
		...
		const openPullRequestsCount = pullRequests.filter(
			(pull: GithubPullRequest) => pull?.state === "open",
		).length;
		...
		const workflowStatus =
			!mostRecentWorkflow?.conclusion ||
			mostRecentWorkflow.conclusion === "success" ||
			mostRecentWorkflow.status === "waiting";
	...
	public async getWorkflowRun(
		repoName: string,
		runId: string | number,
	): Promise<WorkflowRunWithDetails | null> {
		const workflowsRuns = await this.github.getWorkflows(repoName);
		const repo = await this.getRepoData(repoName);

		const workflowRun = workflowsRuns.workflow_runs.find(
			(run) => Number(run.id) === Number(runId),
		);
		...
		return {
			...
			run: {
				...
				conclusion: workflowRun.conclusion as
					| "success"
					| "failure"
					| "cancelled"
					| "skipped"
	...
	public async getWorkflowDetails(
		...
		const jobsData = await this.github.getWorkflowJobs(
			repo.name,
		...
		return {
			...
			run: {
				...
				status: mostRecentWorkflow.status as
					| "queued"
					| "in_progress"
					| "completed",
				conclusion: mostRecentWorkflow.conclusion as
					| "success"
					| "failure"
					| "cancelled"
					| "skipped"
	...
	public async getWorkflowRuns(): Promise<WorkflowRunWithDetails[]> {
		...
		const workflowInfo = await Promise.all(
			repos.map(this.getWorkflowDetails.bind(this)),
		);
