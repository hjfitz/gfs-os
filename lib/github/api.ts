...
export class GithubAPI {
  ...
  public async getPullRequests(repoName: string): Promise<GithubPullRequestResponseDTO> {
    return this.getJson<GithubPullRequestResponseDTO>(`/repos/${this.org}/${repoName}/pulls`)
  }

  public async getWorkflows(repoName: string): Promise<GithubWorkflowRunsResponseDTO> {
    return this.getJson<GithubWorkflowRunsResponseDTO>(`/repos/${this.org}/${repoName}/actions/runs`)
  }
  ...
  public async getWorkflowJobs(repoName: string, workflowRunId: number): Promise<GithubOrgWorkflowJobsResponseDTO> {
    return this.getJson<GithubOrgWorkflowJobsResponseDTO>(`/repos/${this.org}/${repoName}/actions/runs/${workflowRunId}/jobs`)
  }

  public async getJobLogs(repoName: string, jobId: number): Promise<string> {
    const resp = await this.get(`/repos/${this.org}/${repoName}/actions/jobs/${jobId}/logs`, {
      redirect: "follow",
      headers: {
